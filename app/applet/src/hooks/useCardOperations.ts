...
export const useCardOperations = (
  ...
  const addCard = (text: string) => {
    ...
    setCards((prev: Card[]) => {
      ...
      if (currentParentIdRef.current) {
        const parentCard = prev.find(c => c.id === currentParentIdRef.current);
  ...
  const toggleCardState = (id: string) => {
    ...
    setCards((prev: Card[]) => {
      // First, just toggle the state immediately for visual response
      return prev.map((c) =>
        c.id === id ? { ...c, completed: !c.completed } : c,
      );
    });

    // Then sort with a delay so the user sees the card deactivate before it moves down
    setTimeout(() => {
      setCards((prev: Card[]) => {
